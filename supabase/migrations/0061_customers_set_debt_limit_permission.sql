-- 0061_customers_set_debt_limit_permission.sql
--
-- Adds the `customers.set_debt_limit` permission (master plan §18.4: setting a
-- customer's debt limit is CEO/Manager only — distinct from
-- `customers.wallet.update`, which Cashiers also hold for "Add Funds").
--
...
-- =========================================================================
-- 2. New-business seed. Verbatim copy of the 0043 function with one added
--    Manager grant line (customers.set_debt_limit). CEO still gets every key
...
BEGIN
  ...
  -- CEO: all keys (now includes customers.set_debt_limit).
  ...
  -- Manager: explicit list + customers.set_debt_limit (§18.4).
  INSERT INTO public.role_permissions (business_id, role_id, permission_key)
  VALUES
  ...
  -- Cashier: 6 keys (unchanged — Cashiers may Add Funds but not Set Limit).
  INSERT INTO public.role_permissions (business_id, role_id, permission_key)
  VALUES
...
INSERT INTO public.role_permissions (business_id, role_id, permission_key, last_updated_at)
  SELECT business_id, id, 'customers.set_debt_limit', now()
    FROM public.roles
...
--   SELECT COUNT(*) FROM public.permissions WHERE key = 'customers.set_debt_limit'; -- 1
--   SELECT r.slug, COUNT(*) FROM public.roles r
--     JOIN public.role_permissions rp ON rp.role_id = r.id
--    WHERE rp.permission_key = 'customers.set_debt_limit'
