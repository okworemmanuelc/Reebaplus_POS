-- 0160_money_integrity_daily_closings.sql
--
...
-- existing figure. Mirrors the Drift `DailyClosings` table (schema v70 in
...
--
-- DEPLOY ORDER: push this BEFORE the v70 app reaches a device, or the
-- daily_closings upserts the app enqueues would 42P01 (relation does not exist)
-- cloud-side (the land-the-migration-first rule).
--
-- MIGRATION-NUMBER LANE: named 0160 per the cross-branch reservation for #174
-- (schema versions 66–69 + cloud 0155–0159 are held by parallel money-integrity
-- branches). The orchestrator renumbers to contiguous at merge.
...
DECLARE
  ...
  v_tenant_tables   text[] := ARRAY[
    ...
    'stock_counts',
    -- 0160: persisted day close snapshot (#174). FK → businesses + stores +
