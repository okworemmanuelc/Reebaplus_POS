-- migration: 0178_complete_onboarding_without_store
--
-- Issue #232 (parent PRD #229). Store creation leaves sign-up: a new owner
-- finishes the wizard with no Store at all and is walked through creating one
-- inside the app.
--
-- complete_onboarding therefore has to seed a business, its owner, its roles
-- and its settings WITHOUT a store. Two of its eight steps are the store
-- insert (§3) and the owner-to-store binding (§8); both are now guarded on
-- p_store_id being non-null, and the up-front null check is relaxed so only
-- p_business_id is required.
--
-- The parameter list is IDENTICAL to 0123's. Adding a parameter would create a
-- second overload and a PostgREST schema-cache conflict (PGRST203), which has
-- bitten this project before. Because the signature is unchanged and a non-null
-- p_store_id still takes exactly the path it took before, app versions already
-- in the field keep working — which is what makes it safe to deploy this ahead
-- of the client.
...
CREATE OR REPLACE FUNCTION public.complete_onboarding(
  ...
  p_tracks_empty_crates boolean DEFAULT true
)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- p_store_id is optional from #232 onward. A null one seeds the business
  -- without a store; a non-null one behaves exactly as it always has.
  IF p_business_id IS NULL THEN
    RAISE EXCEPTION 'complete_onboarding requires a non-null p_business_id';
  END IF;
  ...
  -- 1. businesses.
  INSERT INTO public.businesses (id, owner_id, onboarding_complete, name, type, phone, email, tracks_empty_crates)
    VALUES (p_business_id, v_uid, true, p_business_name, p_business_type, p_business_phone, p_business_email, p_tracks_empty_crates)
  ON CONFLICT (id) DO UPDATE
    SET name                = EXCLUDED.name,
        ...
        tracks_empty_crates = EXCLUDED.tracks_empty_crates;
  ...
  -- 3. stores — skipped entirely when no store id was supplied (#232).
  ...
  -- 8. Bind the new CEO to their first store — only when there is one (#232).
  --     Without a store the owner has no user_stores row until they create
  --     their first Store in-app, which binds them there instead.
  IF p_store_id IS NOT NULL THEN
