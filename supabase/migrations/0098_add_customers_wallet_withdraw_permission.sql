-- 0098_add_customers_wallet_withdraw_permission.sql
--
-- Adds the `customers.wallet.withdraw` permission (master plan §18.3/§18.4: the
-- "Refund Cash" button on a customer's Wallet card — pays the customer back, in
-- cash, money the business holds for them: a positive spendable wallet credit
-- and/or a held crate deposit once the crates are back). CEO + Manager only,
-- distinct from `customers.wallet.update` (Add Funds, which Cashiers also hold).
--
-- Three idempotent passes (same shape as 0061 / 0069):
...
--
-- The Manager grant list below is copied VERBATIM from 0069 with one added line
-- (customers.wallet.withdraw). The `funds.*` rows are dead grants left from the
-- Funds Register (removed in 0092, master plan §23) — preserved unchanged here
-- to keep this migration a surgical clone; cleaning them up is a separate task.
--
-- Mirror the catalog row in lib/core/database/app_database.dart
-- (`_defaultPermissionRows`, + the v40 onUpgrade INSERT OR IGNORE) so client and
-- cloud stay in step.
...
INSERT INTO public.permissions (key, description, category) VALUES
  ('customers.wallet.withdraw', 'Refund cash from a customer wallet', 'Customers')
ON CONFLICT (key) DO NOTHING;

-- =========================================================================
-- 2. New-business seed. Verbatim copy of the 0069 function with one added
--    Manager grant line (customers.wallet.withdraw). CEO still gets every key
--    via the SELECT, so its block is unchanged.
-- =========================================================================
...
BEGIN
  ...
  -- CEO: all keys (now includes customers.wallet.withdraw via the SELECT).
  ...
  -- Manager: explicit list + customers.wallet.withdraw (§18.4).
  INSERT INTO public.role_permissions (business_id, role_id, permission_key)
  VALUES
  ...
  -- Cashier: 6 keys (unchanged — Cashiers may Add Funds but not refund cash).
  INSERT INTO public.role_permissions (business_id, role_id, permission_key)
  VALUES
...
INSERT INTO public.role_permissions (business_id, role_id, permission_key, last_updated_at)
  SELECT business_id, id, 'customers.wallet.withdraw', now()
    FROM public.roles
...
--   SELECT COUNT(*) FROM public.permissions WHERE key = 'customers.wallet.withdraw'; -- 1
--   SELECT r.slug, COUNT(*) FROM public.roles r
--     JOIN public.role_permissions rp ON rp.role_id = r.id
--    WHERE rp.permission_key = 'customers.wallet.withdraw'
