-- migration: 0123_business_tracks_empty_crates
-- Adds businesses.tracks_empty_crates — per-business opt-in flag for empty-crate
-- / returnable-case tracking. Meaningful only for crate-eligible types
-- (Bar / Beverage distributor). Default true preserves existing tenants.
--
-- Also updates complete_onboarding RPC to accept p_tracks_empty_crates
-- (DEFAULT true) so new onboarding flows can set the flag atomically.

ALTER TABLE public.businesses
  ADD COLUMN IF NOT EXISTS tracks_empty_crates boolean NOT NULL DEFAULT true;

-- Backfill: all existing rows keep crate features (default covers this, but
-- explicit for clarity and idempotency on re-run).
UPDATE public.businesses
   SET tracks_empty_crates = true
 WHERE tracks_empty_crates IS NULL;

-- ─── complete_onboarding (updated with p_tracks_empty_crates param) ───────────
