...
void main() {
  ...
  group('deviceUserIdProvider', () {
    // The provider mirrors the AuthService-owned notifier through the
    // `mirrorNotifier` proxy (#153): Riverpod owns and disposes only the proxy,
    // while the original is merely listened to. So the contract to assert is
    // the two-way value mirror, NOT object identity -- the proxy is by design
    // a distinct instance. Non-disposal of the original is covered by the
    // recompute / force-refresh tests below.
    test('mirrors the AuthService-owned notifier in both directions', () {
      final container = makeContainer();
      final exposed = container.read(deviceUserIdProvider);

      expect(exposed.value, auth.deviceUserIdNotifier.value);

      // service -> exposed
      auth.deviceUserIdNotifier.value = 'from-service';
      expect(exposed.value, 'from-service');

      // exposed -> service
      exposed.value = 'from-provider';
      expect(auth.deviceUserIdNotifier.value, 'from-provider');
    });
