...
| Realtime | Supabase Realtime | Push channel that notifies a device that peer changes exist for its business, triggering a pull. Used as a **signal**, never as the transport for the data itself. Two signal paths run side by side (belt-and-suspenders): the legacy `postgres_changes` bindings, and a writer-agnostic **Broadcast** signal (a generic DB trigger emits a minimal `{table, id, op}` to the tenant topic `store_<business_id>`; the client reacts by scheduling a pull). See §"Live signal — Broadcast". |
...
| Empty-crate movements (customer & supplier) | Drift (SQLite), **append-only** ledgers (`crate_ledger`, `supplier_crate_ledger`); per-owner balance caches (`customer_crate_balances`, `manufacturer_crate_balances`, `store_crate_balances`, `supplier_crate_balances`) | A customer owes US empties; we owe the SUPPLIER empties for the full crates they delivered (§3.13). The signed balance = SUM(quantity_delta); positive on the supplier side = we owe the supplier. Deposit value = outstanding crates × the per-manufacturer deposit rate. **Ledger-as-truth (ADR 0020):** balances are DERIVED from the ledger, never a stored total, and no absolute-value balance row is **pushed**. The `customer_crate_balances` (#158), the **physical empties pool** — `manufacturer_crate_balances` (read #159, push demoted #166) + `store_crate_balances` + the `manufacturers.empty_crate_stock` scalar (#159) — and `supplier_crate_balances` (#160) are all **local-only projections**: the seam still writes them locally but no DAO write path, recovery backfill, or push-priority/natural-key map enqueues them (the scalar is off the `manufacturers` push-column whitelist). Their `SyncedTable` restore entries are retained so a legacy/web/RPC-authored cloud row still restores harmlessly into the (now-unread) projection on pull; every read derives from the ledger. The physical pool per manufacturer is `SUM(quantity_delta)` over the store-stamped, customer-less rows (`store_id IS NOT NULL AND customer_id IS NULL`) — grouped by store for the per-store figure — so the business total always equals Σ store totals. Supplier crate debt derives from `SUM(quantity_delta)` over `supplier_crate_ledger` (`CratePoolDao.watchSupplierCrateDebt`, forwarded from `SupplierCrateBalancesDao.watchBySupplier`), and Receive Stock posts BOTH legs — the physical `crate_ledger` movement AND the `supplier_crate_ledger` movement — for empties returned on a delivery, in one transaction through the seam (#160/B3). With `manufacturer_crate_balances` demoted (#166 — `recordCrateReturnByManufacturer` no longer enqueues it, and it is off the push-priority map), **no crate balance is pushed to the cloud at all**; the ADR 0020 hard contract now holds for every cache plus the scalar. **Count-based Crate Shortage (ADR 0028, #293):** Missing physical empties are derived chronologically per `(manufacturer, store)` by folding counts (`recordManualCountCorrection`). The first count sets an `opening_count` baseline (0 shortage); counts below expected open shortage; counts above expected close shortage first with excess surplus unbanked. All Stores sums open shortages across stores. Surfaced as a warning valued at `shortCount × crateValue` on the `ManufacturerScreen` Short card and `InventoryScreen` brand cards (`⚠ N short` badge), gated by `Gates.countCrates` (`stock.view`). | Ledgers: **Yes**. All balance caches — `customer_crate_balances` / `manufacturer_crate_balances` / `store_crate_balances` / `empty_crate_stock` / `supplier_crate_balances`: **not pushed** (restore-on-pull retained, value unread). |
...
| Day-close snapshots (`daily_closings`, #174) | Drift (SQLite), append / **first-write-only** | The first review of a **finished** calendar day freezes its computed figure set (one row per business × day, natural-key first-writer-wins, deterministic id) so the Daily Reconciliation can render live-vs-reviewed deltas. Purely observational — no money flow changes (ADR 0021 §2). | Yes |
...
**The local database is excluded from OS backups (#285).** Android sets `allowBackup="false"` plus a `dataExtractionRules` file that excludes every domain from **both** cloud backup and device-to-device transfer (`allowBackup` alone does not stop D2D). iOS sets `NSURLIsExcludedFromBackupKey` on `reebaplus_pos.sqlite` and its `-wal` / `-shm` sidecars via the `reebaplus/backup_exclusion` method channel (`BackupExclusionService`, called once at bootstrap). The reason is an integrity one, not a privacy one: a backup restores the Drift database **without** the secure-storage session that scopes it, so a restored or transferred copy can land on a phone whose signed-in identity belongs to a different business — the stale-business state that the sign-in check below exists to clear.

**Clearing one business (`AppDatabase.clearBusinessData`, #285).** `clearAllData` is a whole-device wipe; a phone that holds two businesses needs to lose only one. `clearBusinessData(X)` deletes every row for X across the tables the **schema** says carry a `business_id` column (children first, delete-guards suspended exactly as `clearAllData` does), plus X's `businesses` row, its `sync_queue` rows, its `sync_queue_orphans` rows (matched on the tenant embedded in the payload), and its per-business SharedPreferences keys. Device-level data (the permissions catalogue, app settings) and every other business are untouched, so the business being signed in to keeps its rows, its outbox, and its pull cursor — no re-download. The table list is derived from the schema rather than written by hand so a table added later is covered the day it is added.
...
### A login outlives its business (#285)

Deleting a business does **not** delete the Supabase auth user behind it, so one login can later be attached to a different business while a phone still holds the old one's rows. `resolvePostVerifyRoute` closes that gap at both entry points (email code and Google):

1. `AuthService.fetchSupabaseAccount` returns a three-armed `SupabaseAccountLookup` — **found**, positively **none**, or **unavailable**. The first two are answers; the third is a network failure and is never acted on.
2. On an answer, `clearOtherLocalBusinesses` removes every business on the phone except the one the cloud named (or all of them, when the answer is "none" — which then routes to the no-account screen). Clearing is silent for a tombstoned or outbox-empty tenant and gated behind one warning otherwise; see invariant #12's second carve-out.
3. `StoresDao.getUserByEmail(email, preferredBusinessId:)` treats the preferred business as a **filter, not a tiebreaker** — a row from any other tenant is never handed back, whatever the row count.
4. `_restoreUsers` unhooks `auth_user_id` from any other local row before writing an incoming one, so a leftover row can no longer break the minimum-login pull with `UNIQUE (users.auth_user_id)` (SQLite 2067). The stale row is kept — local history points at it — and the change is local only (`auth_user_id` is off the users push columns).
...
### Live signal — Broadcast (#101) and the two-path signal model

Realtime is a **signal that triggers a pull, never the transport for the data itself** (see the Stack table). What *drives* that pull is deliberately redundant — two independent signal paths run side by side, and a change reaches the engine whichever delivers first:

1. **`postgres_changes`** — one channel with a per-table binding, filtered by `business_id` (PRs #95/#96/#97). The legacy path; kept as best-effort.
2. **Broadcast** (#101) — a writer-agnostic signal emitted from the **database**, so it fires on the row write regardless of who wrote it (mobile outbox push, web `checkout_order` RPC, or the console):
   - **Emit** (`0147_broadcast_sync_signal.sql`): one generic `AFTER INSERT/UPDATE/DELETE` trigger — attached to every business-scoped base table (`+ businesses`) — calls `realtime.send({table, id, op}, 'sync', 'store_<business_id>', private := true)`. The payload is **minimal (no row data)**; the client's reaction to any signal is identical. The trigger is **write-safe** (its whole body is wrapped in `EXCEPTION WHEN OTHERS THEN NULL`, so a signalling failure can never abort the underlying write), fires **last** (named `zz_broadcast_sync_signal`), skips no-op updates, and is `SECURITY DEFINER` so it needs no client `EXECUTE` grant (revoked).
   - **Authorize** (`0148_realtime_broadcast_authorization.sql`): a single `SELECT` RLS policy on `realtime.messages` scopes topic `store_<business_id>` to the caller's own businesses (`current_user_business_ids()`), so a client can only ever subscribe to its own tenant's topic — no cross-tenant signal leak (invariant #5's spirit). There is **no** `INSERT` (send) policy: clients only receive; the trigger emits as a definer and cannot be spoofed by a client.
   - **Subscribe** (client): `CloudTransport.startBroadcast(businessId, {onSignal})` opens one **private** channel on `store_<businessId>` (`realtime.setAuth` before subscribe, as with `postgres_changes`), on the **same lifecycle as realtime** (sign-in / resume / connectivity handoff / logout). Any signal → a **debounced `catchUpPull(reason: 'broadcast')`** (shares the 20 s catch-up debounce, so a burst from one multi-row write collapses to one pull).

**Invariants this preserves.** A Broadcast signal **MUST NOT write Drift** — its only effect is to schedule a pull (the signal callback is handed no row data by construction). Broadcast is **ephemeral (no replay)**: a device that misses a signal (asleep/offline/backgrounded) converges on the next **periodic / reconnect pull**, which is why the periodic pull is retained as the safety net (Workstream C). Adding a new synced tenant table auto-inherits the trigger (the attach loop is DB-driven), so "add a synced table = one registry entry" still holds.
...
12. **The outbox is sacred (offline activity is impossible to lose silently).** A local row that has an unconfirmed `sync_queue` **or** `sync_queue_orphans` entry is **inviolable**: no pull, reconcile, restore-overwrite, or `clearAllData` may delete or overwrite it until its push is **confirmed by the server**. *Sacred means never destroyed silently — not frozen forever.* Un-pushable data must remain **visible and exportable**, and may leave the device **only** by a deliberate, confirmed user action. The enforcement primitive is `SyncDao.pendingRowIds(table, businessId)` — the set of un-uploaded ids for a table across both outbox tables — and every protection is an application of it:
    - **Wipe gate (E):** every `clearAllData` caller checks the outbox first. A salvageable wipe (sole-user logout) with retryable rows is **refused** ("connect and sync first"); with only un-pushable orphans it routes to the **"Resolve unsynced data"** flow (export → typed-confirm discard → logout) and is never trapped. There are exactly two carve-outs, and both record a durable loss breadcrumb (count of rows discarded) so neither is ever silent:
        1. a **business-deleted** wipe — the tenant no longer exists cloud-side, so the data is unsalvageable, and it proceeds with no prompt;
        2. an **older business cleared at sign-in** (#285) — the phone still holds a business the login being verified has left behind, and that tenant's stale `users` row collides with the incoming one on the very first pull. A tombstoned or outbox-empty tenant is cleared with no prompt; a tenant that still exists **and** holds un-uploaded rows shows one warning naming the count and the business, and is cleared only on a deliberate confirmation. Declining abandons the sign-in and leaves the phone untouched. This carve-out is narrower than the logout gate on purpose: the data belongs to a business that is not the one signing in, so "connect and sync first" is advice its owner cannot act on from this phone.
