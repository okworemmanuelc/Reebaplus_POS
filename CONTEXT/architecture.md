...
| Empty-crate movements (customer & supplier) | Drift (SQLite), **append-only** ledgers (`crate_ledger`, `supplier_crate_ledger`); per-owner balance caches (`customer_crate_balances`, `manufacturer_crate_balances`, `store_crate_balances`, `supplier_crate_balances`) | A customer owes US empties; we owe the SUPPLIER empties for the full crates they delivered (§3.13). The signed balance = SUM(quantity_delta); positive on the supplier side = we owe the supplier. Deposit value = outstanding crates × the per-manufacturer deposit rate. **Ledger-as-truth (ADR 0020):** balances are DERIVED from the ledger, never a stored total, and no absolute-value balance row is **pushed**. The `customer_crate_balances` cache (#158), the **physical empties pool** — `store_crate_balances` and the `manufacturers.empty_crate_stock` scalar (#159) — are **local-only projections**: the seam still writes them locally but no DAO write path or recovery backfill enqueues them (the scalar is off the `manufacturers` push-column whitelist; the store cache no longer enqueues). Their `SyncedTable` restore entries are retained so a legacy/web/RPC-authored cloud row still restores harmlessly into the (now-unread) projection on pull; every read derives from `crate_ledger`. The physical pool per manufacturer is `SUM(quantity_delta)` over the store-stamped, customer-less rows (`store_id IS NOT NULL AND customer_id IS NULL`) — grouped by store for the per-store figure — so the business total always equals Σ store totals. `manufacturer_crate_balances` + `supplier_crate_balances` are still pushed absolute until #160 derives + demotes them. | Ledgers: **Yes**. `customer_crate_balances` / `store_crate_balances` / `empty_crate_stock`: **not pushed** (restore-on-pull retained, value unread); `manufacturer_crate_balances` / `supplier_crate_balances`: still Yes until #160. |
