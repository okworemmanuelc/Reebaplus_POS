...
### Money integrity #6 (#174) — persisted day close (snapshot + changed-since-review delta) — CODE-COMPLETE (2026-07-24)
...
- Drift `schemaVersion` 65 → **v70** (66–69 reserved for parallel money-integrity
  branches — orchestrator renumbers at merge). Cloud migration
  **`0160_money_integrity_daily_closings.sql`** (table + RLS via
...
- Tests: `test/dashboard/daily_closings_dao_test.dart` (first-writer-wins, 4),
  ...
  map + delta), `test/database/migration_upgrade_test.dart` (v70 upgrade, 1),
