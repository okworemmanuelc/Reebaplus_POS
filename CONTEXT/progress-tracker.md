...
- **Gate**: new `Gates.addManufacturer` = Manager tier or above AND `products.edit_price`, so a Cashier or Stock keeper given the key through a custom grant still can't reach it. The Crates tab's "Add New" button renders only when it allows. Everyone else doesn't see it at all. The sheet re-checks it with `allowsNow` right before the insert and shows the standard denial message instead of writing (PR #302 review).
- **Save vs. log**: the insert and the activity log are separate steps. A failed insert shows "Could not add manufacturer" and keeps the sheet open. A failed log shows "Manufacturer added, but the activity log could not be saved" and still closes the sheet, so a retry can't create a second manufacturer (PR #302 review).
...
- **Tests**: `test/inventory/add_manufacturer_sheet_test.dart` (13): visibility for CEO, Manager, Cashier, Stock keeper, and a Cashier holding `products.edit_price`, with gates injected; the save-time denial writes nothing; a failed activity log keeps the row and closes the sheet; name + crate value only, and the saved row; blank name and duplicate name rejected with no write; scroll + device-bottom-padding + save at 320x568 and 800x360; Crate Size Group section gone with the row unchanged, and the stats row still showing. `test/inventory/` 95/95, `test/crates` + crate-size-group DB tests + `test/permissions` 378/378, `flutter analyze` clean.
...
- **Cause (three stacked bugs)**: a Supabase login outlives the businesses it belonged to, so the same `auth_user_id` gets reused by a new business. `fetchSupabaseAccount` returned `null` for both "no business" and "couldn't reach the server", so the orphan wipe never ran on a real answer; `getUserByEmail` ignored `preferredBusinessId` whenever exactly one row matched, handing back the stale tenant's row; and `_restoreUsers` matched by id only, so the incoming row was inserted carrying an `auth_user_id` the stale row still held. The exception aborted the whole `syncMinimumLogin` pull, which both screens surfaced as "we could not load your account".
- **Fix (A)**: `fetchSupabaseAccount` now returns a sealed three-armed `SupabaseAccountLookup` (`Found` / `None` / `Unavailable`). `resolvePostVerifyRoute` acts only on the first two; `Unavailable` falls back to the local PIN login, or to the new `AccountLookupUnavailableRoute` (network message) when the phone holds no row — it never routes a real account into sign-up.
- **Fix (A, cont.)**: on a positive answer, `AuthService.clearOtherLocalBusinesses` removes every business except the one the cloud named (all of them when the answer is "none", which then routes to `NoAccountFoundRoute`). Tombstoned or outbox-empty tenants clear with no prompt; a tenant that still exists with un-uploaded rows shows one warning naming the count and the business. Cancelling returns `SignInCancelledRoute` and calls the new `abandonSignIn()` — Supabase + Google sign-out with secure storage untouched. `getUserByEmail` now treats `preferredBusinessId` as a filter, never a tiebreaker.
- **Fix (B)**: new `AppDatabase.clearBusinessData(businessId)` — deletes one tenant across every table the **Drift schema** says carries `business_id` (children first, delete-guards suspended via the extracted `_withDeleteGuardsSuspended`), plus its `businesses` row, both outbox tables, and its per-business prefs keys (`FirstLoadMarkerService.clearMarkerForBusiness`, `SyncCursorResetService.clearForBusiness`). The other business keeps its rows, outbox and pull cursor — no re-download.
- **Fix (C)**: `_restoreUsers` nulls `auth_user_id` on any other local row before writing an incoming one. Local-only; the column is off the users push whitelist. The stale row is kept because local history points at it.
- **Fix (D)**: Android `allowBackup="false"` + `res/xml/backup_rules.xml` + `res/xml/data_extraction_rules.xml` (the latter also covers device-to-device transfer, which `allowBackup` alone does not stop). iOS sets `NSURLIsExcludedFromBackupKey` on the `.sqlite` and its `-wal`/`-shm` sidecars via a new `reebaplus/backup_exclusion` method channel (`BackupExclusionService`, called once from `_bootstrap`).
- **Fix (E)**: both entry screens now show the same plain messages from `sign_in_failure_message.dart` — network vs. "we couldn't finish setting up this phone" — with no raw exception text, and the OTP path records to `CrashReporter` (it recorded nothing before).
- **Docs**: `context/architecture.md` — invariant #12 gains its second carve-out, plus a new "A login outlives its business" subsection under Auth & Access Model and two storage-model notes (backup exclusion, `clearBusinessData`).
- **Tests**: 18 new across `test/auth/post_verify_route_stale_business_test.dart` (11, supersedes `post_verify_route_orphan_business_test.dart`), `test/database/clear_business_data_test.dart` (5, schema-driven sweep over every `sqlite_master` table with a `business_id` column), `test/sync/restore_users_auth_user_id_collision_test.dart` (2).
- **Open question**: `context/code-standards.md` requires all user-facing strings to live in ARB files under `lib/l10n/`, but that directory does not exist and `context.l10n` has zero call sites — the auth screens use raw literals throughout. The #285 strings follow the surrounding code. Either the standard or the codebase needs to move; not resolved here.
...
- **Cause**: the customer card is fixed, and the empty message under it (icon, "Cart is empty", Recall) was a `Center > Column` in an `Expanded` — a block that could not scroll, so a short phone had nowhere to put it.
- **Fix (`lib/features/pos/screens/cart_screen.dart`)**: the empty message is extracted into `_EmptyCartMessage`, a `CustomScrollView` with one `SliverFillRemaining(hasScrollBody: false)` holding it (16dp scaled padding top and bottom — it never decides whether a scroll is needed at a supported size). Centred while there is room; on a short phone it grows past the viewport and one scroll reaches Recall. The customer card stays fixed, so Change is always on screen. Unconditional — no orientation branch, no short-viewport predicate; the shared first-run empty state is untouched. The populated branch is unchanged apart from a `cartLineKey(productId:, name:)` key on each line for the tests (id AND name, as `CartService.addItem` merges lines).
- **Why the harness sweep called Cart unaffected (measured, not guessed)**: `pumpScreen` stands the bottom bar in with a bare 56dp box while the Scaffold still strips the 24dp bottom inset from the body; MainLayout's real `BottomNavigationBar` is `56 + viewPadding.bottom`. At 800x360 the 56dp bar left the empty block **14.6dp spare**; the real 80dp bar overflows it by **9.4px**; test fonts' shorter lines account for the rest of the device's 15px. Correcting the harness default turns four existing tests in other suites red (Inventory empty 800x360, POS grid density 568x320, Supplier Detail scroll-retention + swipe) — each its own slice, so the default is left alone and this suite passes the real height. Recorded as `docs/design/responsive-layout-plan.md` §11.7, along with a second gap: `visibleRowCount` measures a row against the screen, not the scroll view that clips it, so at 800x360 it counted a partly clipped Recall and cart line as complete.
- **Tests** — new `test/pos/cart_viewport_test.dart`, 11 tests: empty × 4 viewports (320x568, 800x360, 915x412, 412x915), empty at 1.3x text on both compact sizes, Recall opens the saved-carts sheet sideways, and with-items × 4 viewports. Each asserts no overflow, the shared content-row assertion (Recall / a cart line), that the target scrolls **wholly** inside the cart's scroll view, and that Change is laid out and hit-testable. The with-items cart is filled before `CartScreen` first builds, as on a phone — filling after the first frame blamed the empty state's overflow on the with-items case. **Red** — the final suite against the unfixed empty branch (line key only): **3 fail / 8 pass** — empty 800x360 (9.4px overflow), empty 800x360 at 1.3x (25px), Recall-opens-sheet (9.4px); every with-items case passes, matching the issue. **Green** after: **11/11**. The wholly-inside check measures against the screen when nothing scrolls, so it judges the defect, not this fix's shape (a first draft required the fix's `CustomScrollView` and failed 4 roomy cases pre-fix for that reason alone). With the scroll-into-view removed, it fails 3 cases at 800x360 that the shared assertion passes. Full suite: all green apart from `test/shared/main_layout_bottom_bar_viewport_test.dart` (#258), which hangs in its first case — reproduced identically on a pristine `origin/main` worktree at `891a748`, so pre-existing. After the fix, at rest Recall needs a scroll only at 800x360 (31.8dp at 1.0x, 47.8dp at 1.3x); everywhere else it fits.
- **Owner emulator pass (2026-09-18): all checks passed** — empty cart upright (message and Recall centred, no band); sideways no band, Recall scrolls wholly into view and opens Saved Carts; Change stays at the top and opens Select Customer; with an item, the line, totals and Proceed to Checkout scroll normally in both orientations. **Follow-up to file**: fix both harness gaps in `test/helpers/screen_harness.dart` (bar height = 56 + bottom inset; complete-row check against the clipping scroll view) and fix the four suites that go red with it; lift the wholly-inside rect check, `settleScreen` and `teardownScreen` (now copied across five viewport suites) into the harness at the same time.
...
- **Changes in `docs/design/responsive-layout-plan.md`**:
  1. **New Section 11 added**: "Audit against runtime measurement — PRD #239 (2026-09-18)" recording the five measured corrections in full detail:
     ...
     - **§11.6 Blocking contradiction flagged**: Plan §4 (rail rejected) vs ADR 0025 §5 (rail required) gave opposite instructions for POS landscape treatment, recorded as an open blocking question halting sales screen work until resolved.
...
- **Emulator finding, fixed in this slice — open dropdowns froze the page** (`lib/shared/widgets/app_dropdown.dart`): the owner reported the page stopping when a dropdown is open. Not a POS defect — `AppDropdown`'s full-screen barrier was a `Container(color: Colors.transparent)`, and a coloured `Container` builds a `ColoredBox` that hit-tests itself even when fully transparent, so it ate the drag despite `HitTestBehavior.translucent`. Now `SizedBox.expand()`. Latent app-wide across **58 dropdowns in 31 files**, invisible only because those screens did not scroll. Also closes the menu on scroll (via `Scrollable.maybeOf(context)?.position`, since scroll notifications go to a scrollable's ancestors and the field is a descendant) — without that, a field in a lazy sliver unmounts when scrolled off and `showWhenUnlinked: false` hides the menu while leaving a live invisible barrier. Pinned by `test/widgets/app_dropdown_scroll_test.dart` (4 tests, two deliberately in tension: the page must scroll AND a tap outside must still dismiss) plus a POS-level case in the viewport suite.
- **Verification (run 2026-09-17, whole suite)**: `flutter analyze` clean (0 errors, 0 warnings); **2171 passed / 271 skipped / 0 failures**.
- **Not caused by this slice — main is red after midnight**: re-running the suite at 00:22 on 2026-09-18 gives **2166 passed / 5 failed**, all in `test/customers/customer_detail_viewport_test.dart` (#245, merged in PR #264). Verified pre-existing by running that file on a pristine `origin/main` worktree at `973d68a`, where it fails identically and repeatably. Cause: the test seeds wallet transactions at `now.subtract(Duration(hours: i))` for i=1..10, so once the clock passes midnight those rows fall on the previous calendar day and the credit history's period filter excludes them — 0 rows at **every** viewport, including the comfortable control, which is why it is a data/time defect and not the starvation defect that suite was written for. Out of scope for #259; needs its own issue against #245's suite (seed relative to a fixed clock, or pin the period filter in the test).
- **Outstanding**: the rest of the owner's emulator pass at 800x360 (mandatory per slice — the #241 harness sweep is not sufficient evidence). #258 is independent; the full sideways experience should be re-checked once both are merged.
...
**MIGRATION NUMBER COLLISION — caught at integration, and it had already bitten.**
#209 landed on main in parallel and took **0173**
(`0173_v2_envelope_crate_credit.sql`), a number this run had already allocated
to #216. `supabase_migrations.schema_migrations` keys on the **version prefix**,
so once `0173` was recorded, `0173_crate_shortfall_writeoff.sql` became
permanently invisible to the CLI — not "would collide", but *did*: the deploy
run's `--dry-run` listed only `0171`, `0172` and `0173_v2_envelope_crate_credit`,
and #216's migration was silently absent. It would never have deployed, with
nothing failing loudly.

Fixed by renumbering **0173→0175** (#216) and **0174→0176** (#217), preserving
relative order so the CREATE still precedes the ALTER that adds its `source`
column; every in-repo reference updated. #209's 0173 was left alone — it
reached main first, and it is deployed.

**DEPLOY STATE (verified on prod, not assumed):**
- **0171, 0172 DEPLOYED** — PRD #203 slices #211/#212. The release gate holds on
  live data: **24/24 manufacturers on `'none'`**, nothing backfilled, **0**
  `supplier_crate_deposits` rows. No tenant's figures moved, which is exactly
