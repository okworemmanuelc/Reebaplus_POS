...
3. On completion, the app auto-creates the four default roles with default permissions, creates the first store, assigns the CEO to it, and lands the CEO on Home (the dashboard). Landing is per role: CEO, Manager and Stock keeper open on Home; Cashier opens on Point of Sale; a Driver gets the driver terminal instead of the shell.
