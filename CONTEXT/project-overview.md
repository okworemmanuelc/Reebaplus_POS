...
- Multi-store data model with a single app-wide active-store picker in this phase, staff multi-store assignment, and a stock-transfer workflow.
...
8. An out-of-stock product appears in the POS grid greyed out and is not tappable, and a Beverage distributor business shows the Empty Crates tab and empty-crate deposit flow that no other business type exposes in this phase.
