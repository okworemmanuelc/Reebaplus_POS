...
**Phase 1**, plus a few **Phase 2 multi-store** slices the user has authorized one at a time (each added to the master plan before building): the stores list/management screen + `stores.manage` permission, the receipt store-address, **staff multi-store assignment** (CEO add/remove of a staff member's `user_stores` set from the staff profile, `staff.assign_stores`, §9.5 — 2026-06-05), the **active-store POS picker** (in-app "pick your store" gate + confine the selling store to assignments, §12.1 — 2026-06-05), and **per-store permission scope** (the Store layer of §10.2.1 — a role's permissions can be overridden per store via `store_role_permissions`; resolver order User > Store > Business; 2026-06-06), and **per-store supplier ledgers** (suppliers stay business-wide, but each supplier ledger entry is stamped with a `store_id`; the active-store picker filters Supplier Accounts balances/history, "All Stores" = aggregate; §21.11 — 2026-06-07), and **per-store expense scope** (each expense is stamped with the active store's `store_id`; the active-store picker filters the Expenses list/Stats/budget, "All Stores" = aggregate for all-stores viewers; the `store_id` column already exists client+cloud, so it's a wiring-only slice; §20.8 — 2026-06-07). Everything else in Phase 2 / Phase 3 is still out of scope — if you find yourself reaching for an unauthorized one (per-store report filters, stock-transfer UI, custom roles, etc.), stop and ask.
...
**Two things `MainLayout`'s Scaffold does to EVERYTHING under it (both proven on-device — do not relearn them):** Its app nav bar is **never null** (`SizedBox.shrink()` when hidden) and `resizeToAvoidBottomInset` defaults **true**. So for every screen, pushed route, and modal on a tab Navigator:

1. **It zeroes the bottom MediaQuery.** `padding.bottom` **and** `viewInsets.bottom` both read **0** (`scaffold.dart`: `removeBottomPadding` / `removeViewInsets` when a `bottomNavigationBar` is present). So `MediaQuery.padding.bottom`, `context.bottomInset`, and `SafeArea(top: false)` / `SafeArea(bottom: true)` all silently read **0** — content paints under the nav bar even when "fixed" with them.
2. **It already resizes the body UP by the keyboard.** The whole tab area shrinks above the keyboard, so the keyboard is **already handled** for everything inside (screens, pushed routes, modals alike) — your content must **NOT** add it again.

**The reliable inset:** **`context.deviceBottomPadding`** (in [responsive.dart](lib/core/utils/responsive.dart)) — the **system-nav inset ONLY**, recomputed from the raw `FlutterView` (`MediaQueryData.fromView(View.of(context))`), which **no `Scaffold` can zero out**. Keyboard down → it clears the nav bar; keyboard up → it collapses to 0 (point 2 covers the keyboard). Correct on 3-button nav (large), gesture nav (thin pill), and iOS.

**Do NOT use `context.deviceBottomInset` (nav + keyboard).** Because of point 2 it **double-counts** the keyboard and the content leaps up "like a second keyboard" when a field is focused — visible in fixed `Column`s / footer slots, wasteful scroll-extent inside scrollables. It was the documented choice until **2026-06-07**, when the checkout crate-deposit sheet proved the double-count on-device; **all 74 call sites were converted to `deviceBottomPadding`** and the codebase has none left. `deviceBottomInset` is correct **only** for content NOT under MainLayout's resize (`useRootNavigator: true` routes, pre-login auth) — the app has none today. (A screen's own `resizeToAvoidBottomInset: false` is a harmless **no-op** under MainLayout — it sees `viewInsets 0`, so its own resize does nothing; MainLayout's resize does the lifting. Several screens still carry the flag; leave it.)

**The decision rule — ask: "when this widget is on screen, does it reach the PHYSICAL screen bottom?"**

- **YES → use `context.deviceBottomPadding`.** Covers: anything inside `showModalBottomSheet` / `showDialog` / `DraggableScrollableSheet` (modals always hide the bar); pushed detail screens (footers, scrollable bottoms); and drawer-accessed tab roots where the bottom nav bar is hidden — Customers, Payments, Expenses, Stores, Deliveries, Activity Log.
- **NO → leave it (0 is correct).** This is **only** the body of the five bottom-nav tab roots rendered above the visible bar — **Home, POS, Inventory, Orders (list), Cart**. The bar already insets them; adding any bottom inset makes a **gap above the bar**. Their existing `context.bottomInset` reads 0 there, which is correct — do **not** convert it. (But modals opened *from* these files still use `deviceBottomPadding`.)
- **Floating action buttons (`AppFAB`):** same YES/NO split. `AppFAB` already lifts itself via `reserveBottomInset` (default **true**, nav-only `context.deviceBottomPadding`) — the Scaffold's `floatingActionButton` slot does **not** add the system-nav inset on edge-to-edge (only a ~16px margin), so on a **3-button nav** the bar paints over the FAB (the gap is ~invisible on gesture nav, which is why this was missed at first). Set `reserveBottomInset: false` only on the visible-bar tab roots (POS, Stock), exactly like the NO rule above. Never wrap an `AppFAB` in your own bottom inset.
...
**`showModalBottomSheet` — the `useSafeArea` trap (also a real bug):** `useSafeArea: true` wraps the sheet in `SafeArea(bottom: false)` — it protects top/left/right only and **does NOT inset the bottom**. Combined with the MediaQuery-zeroing above (even a nested `SafeArea(bottom)` reads 0 under `MainLayout`), the bottom is **always** your job — put `context.deviceBottomPadding` on the footer padding.

**Never:**

- Use `MediaQuery.padding.bottom`, `context.bottomInset`, or a bottom `SafeArea` for bottom-anchored content under `MainLayout` — they read **0**. Use `context.deviceBottomPadding`.
- Use `context.deviceBottomInset` anywhere under `MainLayout` — it double-counts the keyboard (MainLayout's Scaffold already resizes for it), throwing content up "like a second keyboard." Use `deviceBottomPadding`.
- Add a bottom inset to the **body** of one of the five bottom-nav tab roots (Home / POS / Inventory / Orders / Cart) — it creates a gap above the bar. (Their modals are fine.)
- Assume `showModalBottomSheet(useSafeArea: true)` insets the bottom — it doesn't (`SafeArea(bottom: false)`).
- Double-pad: don't add an inset to content already in a Scaffold's `bottomNavigationBar` / `persistentFooterButtons` / `bottomSheet` slot. (`floatingActionButton` is deliberately NOT in this list — the Scaffold does not inset it for the system nav on edge-to-edge; `AppFAB` owns that inset itself via `reserveBottomInset`, see the decision rule above.)
- Run the raw inset through the size scaler (`getRSize(deviceBottomPadding + …)`). Apply the raw inset and scale only the fixed gap: `context.deviceBottomPadding + context.getRSize(16)`.
